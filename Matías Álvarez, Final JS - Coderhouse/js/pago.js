    ...
    $("#form-carrito").submit(function (e) {
        if ($("#nombre").val() == "") {
            ...
            if (($("#num-tarjeta").val() == "") || ($("#num-tarjeta").val().length != 16)) {
                ...
                //Es momento de convertir el array a formato JSON. Esto es importante ya que como nos enseñaste. Si usamos el JSON, debemos usarlo para que tenga sentido
                let datosCompraJSON = JSON.stringify(datosCompra);
                enviarDatos(datosCompraJSON);

                //Creamos un vinculo afectivo con el cliente al notificandolo de su compra
                Swal.fire({
                    icon: 'success',
                    title: '¡Felicidades, tu compra ha sido un procesada  ',
                    text: 'A la brevedad recibiras mail de confirmación con el detalle de la compra. Como una susprición gratuita a Guitaramigos',
                    confirmButtonColor: "🛍️"
                });

                //vacio el carrito. invocamos función
                vaciarCarrito();
            ...
            //En caso de que el cliente eliga un método de pago en efectivo, tenemos esta condicional
            e.preventDefault();

            //Ya con los datos validados, procedemos a guardarlo en un array 💪💪
            let datosCompra = [];
            datosCompra.push($("#nombre").val());
            datosCompra.push($("#email").val());
            datosCompra.push($("#telefono").val());
            ...
            //una vez que guardamos el array, lo conviertimos a formato JSON (Clase 11)
            let datosCompraJSON = JSON.stringify(datosCompra);
            enviarDatos(datosCompraJSON);

            //Notificamos al usuario que la compra fue exitosa 
            Swal.fire({
                icon: 'success',
                title: '¡Compra confirmada!',
                text: 'Pronto te llegara un mail de la compra en tu casilla - Coder Guitar',
                confirmButtonColor: "🛍️🛍️🛍️🛍️"
            });
